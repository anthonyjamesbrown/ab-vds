function Get-VDSNodeMonitor
{
    <#
        ...
        .DESCRIPTION
            ...
            The following data is returned:
                ...
                isControlPanelRunning            : A value of true indicates that the Jetty Web server hosting the Control Panel is running.  A Value
                                                    of false indicates that the Jetty Web server is not running.
                isLcsLeader                      : A value of true indicates GlassFish is running on the node.  A value of false, indicates GlassFish is not 
                                                    running on the node.
                isVdsLeader                      : A value of true indicates the VDS service running on the node is the current cluster leader.  A value of false
                                                    indicates the VDS service running on the node is not the cluster leader.
                isVdsRunning                     : A value of true indicates the VDS service is running on the node.  A value of false indicates that the VDS
                ...
                systemLoadAverage                : The system load average for the last minute.  The system load average is the sum of the munber of runnable
                                                    entities queued to the available processors averaged over a period of time.  The way in which the load
                                                    average is calculated is operating system specific but is typically a damped time-dependent average.
                systemOpenFileDescriptors        : Number of currently open file descriptors.  (only availabe when deployed on Linux, Sorry!)
                systemOpenFileDescriptorsPercent : Current percentage of available open file descriptors in use.  (only available when deployed on Linux, Sorry!)
        ...
        .EXAMPLE
            PS> Get-VDSNodeMonitor | Where-Object name -eq wprsedifvds001v.asurion.org
        ...
        .EXAMPLE
            PS> Get-VDSNodeMonitor -NodeId a80969d3-0f8c-4e0f-9a09-0579e92023f1
    ...
    $LevelOneProps | ForEach-Object {
        ...
        $SubNode | ForEach-Object {
            ...
            $Object | Add-Member -NotePropertyName 'Name' -NotePropertyValue $Node

            $Object

        } # end foreach
    } # end foreach
} # end function Get-VDSNodeMonitor
