...
Describe "Unit Test: $FunctionName" {
    ...
    It "Test $FunctionName`: NodeName should be returned for the supplied CloudId." {

        mock 'Invoke-VDSAdminAPIRestMethod' -MockWith { $JsonObject | ConvertFrom-Json }

        $Result = Get-VDSNodeName -Server 'wprsedifvds003v' -CloudId 'aaeccabc-7af2-4fcf-b2db-020690112f5c'
        $Result | Should -Be 'wprsedifvds003v.asurion.org'
    }

    It "Test $FunctionName`: An exception should be thrown for an invalid CloudId." {

        mock 'Invoke-VDSAdminAPIRestMethod' -MockWith { $JsonObject | ConvertFrom-Json }

        { Get-VDSNodeName -Server 'wprsedifvds003v' -CloudId 'wprsedifvds003v.wrong' } | Should -Throw
    }
}